...
if (!["general", "gameplay", "tools"].includes(normalizedTarget)) {
  throw new Error(
    `Unknown resume target "${normalizedTarget}". Use general, gameplay, or tools.`,
  );
}
const theme =
  ...
  DEFAULT_RENDER_CV_THEME;
const buildDir = join(".resume-build", normalizedTarget);
...
const pdfFileName =
  normalizedTarget === "general"
    ? "resume.pdf"
    : `resume-${normalizedTarget}.pdf`;
const pdfPath =
  normalizedTarget === "general"
    ? canonicalPdfPath
    : join(pdfOutputDir, pdfFileName);
...
console.log(`Building resume for target: ${normalizedTarget}`);
